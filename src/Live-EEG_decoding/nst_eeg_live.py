...
class NST_EEG_LIVE(Dataset):
    ...
    def __init__(self, base_dir, file_name, **kwargs):
        ...
        super(NST_EEG_LIVE, self).__init__(**kwargs)
    ...
    def load_from_mat(self, labels, trials, raw_data, fs, **kwargs):
        
        self.raw_data = raw_data[:,0:3]
        self.labels = np.array([labels]).flatten() - 1  #labels - 1 #to be consistent with load()
        ### subtraction of trial offset already happens in record_data_liveEEG
